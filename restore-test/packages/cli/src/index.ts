...
import { buildSnapshot } from 'julion-engine-snapshot';
import { restoreSnapshot } from 'julion-engine-restore';
import { authenticate, uploadSnapshot, downloadSnapshot } from 'julion-cloud-google-drive';
...
program
  ...
  .action(async (options: { out?: string }) => {
    ...
    const adapterName = adapter?.adapterName || 'generic';
    const snapshotPath = options.out || path.join(projectRoot, `${path.basename(projectRoot)}.on`);

    try {
      const archive = await buildSnapshot(projectRoot, snapshotPath, adapterName);
